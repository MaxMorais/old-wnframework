 ...
 */
var erpnext={};var wn={};erpnext.send_message=function(opts){wn.call({type:"POST",method:"website.helpers.contact.send_message",args:opts,callback:opts.callback});}
...
if(data.exc){if(opts.btn){$(opts.btn).addClass("btn-danger");setTimeout(function(){$(opts.btn).removeClass("btn-danger");},1000);}}else{if(opts.btn){$(opts.btn).addClass("btn-success");setTimeout(function(){$(opts.btn).removeClass("btn-success");},1000);}}
if(opts.msg&&data.message){$(opts.msg).html(data.message).toggle(true);}
if(opts.callback)
opts.callback(data);}});return false;}
...
$("#user-tools a").tooltip({"placement":"bottom"});$("#user-tools-post-login a").tooltip({"placement":"bottom"});});function valid_email(id){if(id.toLowerCase().search("[a-z0-9!#$%&'*+/=?^_`{|}~-]+(?:\.[a-z0-9!#$%&'*+/=?^_`{|}~-]+)*@(?:[a-z0-9](?:[a-z0-9-]*[a-z0-9])?\.)+[a-z0-9](?:[a-z0-9-]*[a-z0-9])?")==-1)
...
window.location.href="login";}else{wn.call({type:"POST",method:"website.helpers.cart.update_cart",args:{item_code:opts.item_code,qty:opts.qty,with_doclist:opts.with_doclist},btn:opts.btn,callback:function(r){if(opts.callback)
opts.callback(r);}});}},});
/*
